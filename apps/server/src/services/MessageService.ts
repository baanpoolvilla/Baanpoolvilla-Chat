...
export class MessageService {
  ...
  /**
   * Record a broadcast that was sent to a contact as an outbound message in their
   * conversation, so it shows up in the chat like a normal admin message.
   * Failures are swallowed (logged only) — the broadcast itself already succeeded.
   */
  static async recordBroadcastMessage(params: {
    contactId: string;
    platform: Platform;
    ...
    try {
      // Reuse the most recent conversation for this contact+platform; create one if none exists
      const existing = await prisma.conversation.findFirst({
        where: { contactId: params.contactId, platform: params.platform },
      ...
      if (existing) {
        ...
        const created = await prisma.conversation.create({
          data: {
            ...
            channelId: params.channelIdFallback,
            status: 'OPEN',
            isBot: false,
            lastMessage: params.content.substring(0, 200),
            lastMsgAt: new Date(),
      ...
      const message = await prisma.message.create({
        data: {
          ...
          contentType: params.contentType,
          mediaUrl: params.mediaUrl,
          metadata: { broadcastId: params.broadcastId } as Prisma.InputJsonValue,
        },
        select: messageWithReplySelect,
      });
      ...
      logger.error('recordBroadcastMessage failed', {
        error,
        contactId: params.contactId,
